...
var options = null;
        ...
        var defaultMetricsToTrack = ['playbackSummary', 'playTime'];
        ...
        var playIntervals = {
            ...
            getTotalPlayTime: function () {
                ...
                //console.log(this.arrayOfTimes.length);
                var TotalPlayTime = 0;
        ...
        var timeupdate = function () {
            if (streamId) {
                ...
                if (metricsToTrack.playTime && currentTime > 0 && currentTime % 5 == 0 && player.lastTime != currentTime) {
                    console.log('playtime ' + currentTime);
                    submitPlaybackSummary();
                    player.lastTime = currentTime; //prevent sending same tracking over same amount of seconds
                }
        ...
        function unloadData() {
            ...
            if (!player.isLive()) {
                totalPlayTime = Math.round(player.currentTime());
                totalFullscreenTime = playIntervals.totalSecondsFullscreen;
                totalPercentViewed = Math.min(Math.round((totalPlayTime / player.duration()) * 100), 100);
            }
        ...
        function submitPlaybackSummary() {
            var totalPlayTime = playTimeLive.totalSeconds;
            var totalFullscreenTime = playTimeLive.totalSecondsFullscreen;
            var totalPercentViewed = Math.min(percentPlayed, 100);
        ...
        player.addEventListener("durationchange", submitPlaybackSummary);
        player.addEventListener("firstquartile", submitPlaybackSummary);
        player.addEventListener("thirdquartile", submitPlaybackSummary);
        player.addEventListener("midpoint", submitPlaybackSummary);
        player.addEventListener("complete", submitPlaybackSummary);
        player.addEventListener("ended", submitPlaybackSummary);
        ...
        if (metricsToTrack.playTime || metricsToTrack.bitrateQuality || metricsToTrack.playbackSummary) {
            window.addEventListener("onunload", exit, false);
